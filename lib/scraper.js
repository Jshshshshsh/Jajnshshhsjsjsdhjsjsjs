...
//const axios = require('axios');
//const cheerio = require('cheerio');
//let pageUrl = 'https://pin.it/3K55t2w5n';

//const axios = require('axios');
//const cheerio = require('cheerio');

async function fetchAndConvertVideo(url) {
    try {
        if (url.includes('https://pin.it/')) {
            console.log('Extracting original pin link');
            const shortUrlResponse = await axios.get(url); // Espera pela resposta usando await
        ...
        console.log('Fetching content from given URL');
        const body = await axios.get(url); // Espera pela resposta usando await
        if (body.status !== 200) {
            console.log('Entered URL is invalid or not working.');
            return;
        }
        ...
        console.log('Video URL:', convertUrl);
        return { videoUrl: convertUrl }; // Retorna o URL convertido
    } catch (e) {
        console.log('Error:', e.message);
        throw e; // Lança o erro para ser capturado no `catch` externo
    }
}
...
//const axios = require('axios');
//const cheerio = require('cheerio');
async function letraMusica(query)  {
...
//const q = 5; // Substitua pelo valor desejado
//const sender = "id_do_sender"; // Substitua pelo ID do remetente
//const isGroup = true; // Substitua com base na lógica de grupo ou não

//figuss(q, sender, client, isGroup);

  

module.exports = {wallpaper, fetchAndConvertVideo, wikimedia, porno, hentai, quotesAnime, igstalk,aiovideodl, umma, ringtone, letraMusica, happymod, xnxx, pinterest , animesearch}
