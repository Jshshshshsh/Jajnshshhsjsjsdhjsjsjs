...
router.get("/aleatorios/attpv2", async (req, res, next) => {
  const apikey = req.query.apikey;
  const query = req.query.query;
  const outputPath = path.join(__dirname, '..', 'tmp', 'attp.gif'); // Assumindo que a pasta 'tmp' exista
  ...
  if (!apikey) return res.json(loghandler.notparam);
    if (keyLimits[apikey] && keyLimits[apikey].valid) {
    try {
      const result = await attpv2(query, outputPath, './lib/font_attp/Peteroy.ttf');
      res.sendFile(outputPath);
    } catch (error) {
      res.json({ success: false, message: error.message });
    }
...
router.get("/aleatorios/attpv3", async (req, res, next) => {
  const apikey = req.query.apikey;
  const query = req.query.query;
  const outputPath = path.join(__dirname, '..', 'tmp', 'attp.gif'); // Assumindo que a pasta 'tmp' exista
  ...
  if (!apikey) return res.json(loghandler.notparam);
    if (keyLimits[apikey] && keyLimits[apikey].valid) {
    try {
      const result = await attpv3(query, outputPath, './lib/font_attp/edosz.ttf');
      res.sendFile(outputPath);
    } catch (error) {
      res.json({ success: false, message: error.message });
    }
...
router.get("/aleatorios/attpv4", async (req, res, next) => {
  const apikey = req.query.apikey;
  const query = req.query.query;
  const outputPath = path.join(__dirname, '..', 'tmp', 'attp.gif'); // Assumindo que a pasta 'tmp' exista
  ...
  if (!apikey) return res.json(loghandler.notparam);
    if (keyLimits[apikey] && keyLimits[apikey].valid) {
    try {
      const result = await attpv4(query, outputPath, './lib/font_attp/Minecraft.ttf');
      res.sendFile(outputPath);
    } catch (error) {
      res.json({ success: false, message: error.message });
    }
